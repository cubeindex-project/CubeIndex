	...
	<div class="mx-auto flex max-w-7xl items-center justify-between px-2 py-4">
	  ...
	  <nav class="hidden items-center gap-8 md:flex">
		...
		{#if user && profile}
		  ...
		  <button
			on:click={signOut}
			class="rounded-xl bg-red-600 px-4 py-2 text-sm transition hover:bg-red-700"
		  >
			Sign Out
		  </button>
		{:else}
		...
		<ul class="flex flex-col gap-3">
		  ...
		  {#if user && profile}
			...
			<li>
			  <button
				on:click={signOut}
				class="block w-full rounded-xl bg-red-600 py-2 text-center text-white transition hover:bg-red-700"
			  >
				Sign Out
			  </button>
			</li>
