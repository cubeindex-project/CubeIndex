...
import { createCubeSchema } from "$lib/components/validation/cubeForm.js";
import { logError } from "$lib/server/logError";
import { createLogger } from "$lib/server/logger";
import { extractVendorHostnames } from "$lib/components/helper_functions/vendorHosts.js";
...
export const load: PageServerLoad = async ({ params }) => {
	...
	const imageHostnames = extractVendorHostnames(vendors ?? []);
	if (imageHostnames.length === 0) {
		return logError(
			500,
			"No vendor hostnames configured for image validation",
			log,
			new Error("Missing vendor hostnames"),
		);
	}
	...
	const form = await superValidate(
		...
		zod4(createCubeSchema(imageHostnames)),
		{ errors: false }
	);
...
export const actions: Actions = {
	default: async ({ request, locals }) => {
		...
		if (vendorErr) {
			return logError(500, "Failed to load vendor list", locals.log, vendorErr);
		}
		...
		const form = await superValidate(request, zod4(createCubeSchema(imageHostnames)));
