...
<SsgoiTransition id={page.url.pathname}>
  <section class="min-h-screen">
    <div class="grid grid-cols-1 md:grid-cols-2 min-h-screen">
      <!-- Left: Signup process -->
      <div class="h-full flex flex-col justify-center px-6 sm:px-12 py-10 bg-base-200 border-base-300 md:border-r border-y">
        <div class="w-full max-w-md mx-auto">
        <h1 class="text-3xl font-clash font-bold mb-2">Join CubeIndex</h1>
        <p class="text-sm mb-6">Create a free account to start tracking your collection</p>

        <!-- Stepper -->
        <ol class="steps steps-vertical sm:steps-horizontal w-full mb-6 sm:mb-8 text-xs sm:text-sm">
          {#each steps as s}
            <li class="step" class:step-primary={isActive(s.key) || isDone(s.key)}>
              {s.label}
            </li>
          {/each}
        </ol>
      ...
      <aside class="hidden md:flex h-full relative overflow-hidden border-l border-base-300 bg-base-100">
        <!-- Background gradient -->
        <div class="absolute inset-0 bg-gradient-to-br from-primary/20 via-secondary/20 to-accent/20"></div>

        <!-- Decorative blobs -->
        <div class="pointer-events-none absolute -top-20 -right-10 w-72 h-72 bg-primary/30 blur-3xl rounded-full"></div>
        <div class="pointer-events-none absolute -bottom-24 -left-8 w-80 h-80 bg-secondary/20 blur-3xl rounded-full"></div>

        <div class="relative z-10 p-8 flex flex-col justify-center gap-6 w-full">
          <div>
            <h2 class="text-2xl sm:text-3xl font-clash font-extrabold tracking-tight">
              All your cubes, in one place
            </h2>
            <p class="mt-2 text-sm opacity-80">
              Track your collection, rate cubes, unlock achievements, and get price updates.
            </p>
          </div>

          <ul class="space-y-3 text-sm">
            <li class="flex items-start gap-3">
              <span class="badge badge-primary"><i class="fa-solid fa-cubes"></i></span>
              <div>
                <div class="font-semibold">Collection management</div>
                <div class="opacity-70">Add cubes, set condition and status, and mark mains.</div>
              </div>
            </li>
            <li class="flex items-start gap-3">
              <span class="badge badge-warning"><i class="fa-solid fa-star"></i></span>
              <div>
                <div class="font-semibold">Ratings & reviews</div>
                <div class="opacity-70">Share your experience and see what others think.</div>
              </div>
            </li>
            <li class="flex items-start gap-3">
              <span class="badge badge-accent"><i class="fa-solid fa-trophy"></i></span>
              <div>
                <div class="font-semibold">Achievements</div>
                <div class="opacity-70">Unlock badges as you grow your collection.</div>
              </div>
            </li>
          </ul>

          <div class="mt-4 grid grid-cols-3 gap-3">
            <div class="aspect-[4/3] rounded-xl bg-base-100/70 border border-base-300 shadow-sm"></div>
            <div class="aspect-[4/3] rounded-xl bg-base-100/70 border border-base-300 shadow-sm"></div>
            <div class="aspect-[4/3] rounded-xl bg-base-100/70 border border-base-300 shadow-sm"></div>
          </div>
        </div>
