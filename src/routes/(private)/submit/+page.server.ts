...
import { createCubeSchema } from "$lib/components/validation/cubeForm";
import { logError } from "$lib/server/logError";
import { extractVendorHostnames } from "$lib/components/helper_functions/vendorHosts";
...
export const load = (async ({ locals }) => {
	const [
		...
		locals.supabase
			...
			.select("name")
			.order("name", { ascending: true }),
		locals.supabase.from("vendors").select("base_url"),
	]);
	...
	if (vendorErr) {
		locals.log.error({ err: vendorErr.message }, "Failed to fetch vendors");
		throw error(500, "Failed to fetch vendors");
	}

	const allowedImageHostnames = extractVendorHostnames(vendors ?? []);
	if (allowedImageHostnames.length === 0) {
		locals.log.error(
			{},
			"No vendor hostnames configured for image validation",
		);
		throw error(500, "No vendor hostnames configured for image validation");
	}
	...
	const form = await superValidate(zod4(createCubeSchema(allowedImageHostnames)), {
		errors: false,
	});
	...
	return {
		...
		surfaces,
		subTypes,
		imageHostnames: allowedImageHostnames,
	};
...
export const actions: Actions = {
	default: async ({ request, locals }) => {
		...
		const { data: vendors, error: vendorErr } = await locals.supabase
			.from("vendors")
			.select("base_url");
		...
		const form = await superValidate(request, zod4(createCubeSchema(allowedImageHostnames)));
