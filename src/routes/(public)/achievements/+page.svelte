<script lang="ts">
	...
	function idOfUser(user: string) {
		const profile = profiles?.find((p: { username: string }) => p.username === user);
			...
			{#each achievements as achievement}
				...
				>
					...
					<div class="flex-1 p-6">
						...
						<h2 class="text-xl font-bold mb-1">{achievement.name}</h2>
						...
						<div class="text-xs text-gray-400 space-y-1">
							...
							<a href={idOfUser(achievement.submitted_by)}>
								...
								<span class="underline">{achievement.submitted_by}</span>
							</a>
							<p>
