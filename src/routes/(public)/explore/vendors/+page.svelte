...
{#if databaseAvailability}
    <section class="min-h-screen bg-black text-white px-6 py-16">
        <div class="max-w-6xl mx-auto">
            <h1
                class="text-4xl sm:text-5xl font-clash font-bold mb-10 text-center"
            >
                Explore Vendors
            </h1>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                {#each vendors as vendor}
                    <a
                        ...
                        class="bg-neutral-900 border border-neutral-700 rounded-2xl shadow-lg overflow-hidden flex flex-col items-center p-6 transition hover:bg-neutral-800 hover:border-neutral-500"
                    >
                        {#if vendor.logo_url}
                            <img
                                src={vendor.logo_url}
                                alt={`${vendor.name} logo`}
                                class="p-2 h-24 object-contain rounded-xl bg-neutral-800 mb-4"
                            />
                        {:else}
                            <div
                                class="h-20 w-20 rounded-xl bg-neutral-800 mb-4 flex items-center justify-center text-3xl text-blue-400 font-bold"
                            >
