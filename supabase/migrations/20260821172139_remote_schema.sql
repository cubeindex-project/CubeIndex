...
alter table "public"."cube_vendor_links" drop column "cube_slug";

alter table "public"."cube_vendor_links" drop column "vendor_name";

alter table "public"."cube_vendor_links_snapshot" drop column "cube_slug";

alter table "public"."cube_vendor_links_snapshot" drop column "vendor_name";
...
create or replace view "public"."v_price_history" as  WITH daily_rows AS (
         SELECT DISTINCT ON (snapshot.cube_id, snapshot.vendor_id, ((snapshot.created_at)::date)) snapshot.cube_id,
            snapshot.vendor_id,
            vendor.name AS vendor_name,
            (snapshot.created_at)::date AS day,
            snapshot.price
           FROM (public.cube_vendor_links_snapshot snapshot
             JOIN public.vendors vendor ON ((vendor.id = snapshot.vendor_id)))
          ORDER BY snapshot.cube_id, snapshot.vendor_id, ((snapshot.created_at)::date), snapshot.created_at DESC
        )
 SELECT daily_rows.cube_id,
    daily_rows.vendor_id,
   ...
   FROM daily_rows
  GROUP BY daily_rows.cube_id, daily_rows.vendor_id, daily_rows.vendor_name;
