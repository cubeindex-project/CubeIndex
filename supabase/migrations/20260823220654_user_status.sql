...
alter type "public"."user_cube_status" rename to "user_cube_status__old_version_to_be_dropped";

create type "public"."user_cube_status" as enum ('owned', 'wanted', 'previously_owned');

alter table "public"."user_cubes" alter column status type "public"."user_cube_status" using (
  case status::text
    when 'Owned' then 'owned'
    when 'Borrowed' then 'owned'
    when 'Wishlist' then 'wanted'
    when 'Loaned' then 'previously_owned'
    when 'Lost' then 'previously_owned'
  end
)::"public"."user_cube_status";

drop type "public"."user_cube_status__old_version_to_be_dropped";

alter table "public"."user_cubes" add column "bought_from_id" bigint;

update user_cubes uc set bought_from_id = (select id from vendors v where v.slug = uc.bought_from);

alter table "public"."user_cubes" drop column "bought_from";

alter table "public"."user_cubes" add constraint "user_cubes_bought_from_id_fkey" FOREIGN KEY (bought_from_id) REFERENCES public.vendors(id) ON UPDATE CASCADE ON DELETE SET DEFAULT not valid;

alter table "public"."user_cubes" validate constraint "user_cubes_bought_from_id_fkey";

alter table "public"."user_cubes" add constraint "no_quantity_for_wishlist" CHECK (((status <> 'wanted'::public.user_cube_status) OR (quantity = 1))) not valid;
...
create or replace view "public"."v_detailed_cube_models" as  SELECT cm.name,
    ...
    ( SELECT count(*) AS count
           FROM public.user_cubes uc
          WHERE ((uc.cube_id = cm.id) AND (uc.status = 'wanted'::public.user_cube_status))) AS wishlist_count,
    ( SELECT count(*) AS count
           FROM public.user_cubes uc
          WHERE ((uc.cube_id = cm.id) AND (uc.status = 'previously_owned'::public.user_cube_status))) AS previously_owned_count,
...
create or replace view "public"."v_detailed_vendors" as  SELECT v.id,
    ...
    ( SELECT count(DISTINCT uc.user_id) AS count
           FROM public.user_cubes uc
          WHERE (uc.bought_from_id = v.id)) AS buyer_count
...
create or replace view "public"."v_user_stats" as  WITH cube_stats AS (
         SELECT uc.user_id,
           ...
           FROM public.user_cubes uc
          WHERE (uc.status <> 'wanted'::public.user_cube_status)
          GROUP BY uc.user_id
        ), rating_stats AS (
         ...
         SELECT uc.user_id,
            ...
            count(*) AS cnt
           FROM (public.user_cubes uc
             JOIN public.vendors v ON ((v.id = uc.bought_from_id)))
          WHERE (uc.status <> 'wanted'::public.user_cube_status)
         ...
         SELECT user_cubes.user_id,
            ...
            count(*) AS cnt
           FROM public.user_cubes
          WHERE ((user_cubes.acquired_at IS NOT NULL) AND (user_cubes.status <> 'wanted'::public.user_cube_status))
