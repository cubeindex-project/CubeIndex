...
values
	('11111111-1111-4111-8111-111111111111', (select id from cube_models where slug = 'gan-16-maglev'), true, 1, 'Good', 'owned', 'Current competition main.', '2026-02-14', (select id from vendors v where v.slug = 'thecubicle'), 62.99, 'USD', 7850),
	('11111111-1111-4111-8111-111111111111', (select id from cube_models where slug = 'moyu-rs3-m-v5'), false, 2, 'Good', 'owned', 'One stock and one set up for OH.', '2025-11-10', (select id from vendors v where v.slug = 'speedcubeshop'), 19.90, 'EUR', 13240),
	('11111111-1111-4111-8111-111111111111', (select id from cube_models where slug = 'yj-mgc-square-1'), false, 1, 'New', 'wanted', 'Try before the next competition.', null, null, null, null, null),
	('22222222-2222-4222-8222-222222222222', (select id from cube_models where slug = 'moyu-weilong-wr-m-v10'), true, 1, 'New', 'owned', 'Smooth setup with medium compression.', '2026-03-02', (select id from vendors v where v.slug = 'speedcubeshop'), 34.95, 'GBP', 9430),
	('22222222-2222-4222-8222-222222222222', (select id from cube_models where slug = 'qiyi-ms-2x2'), false, 1, 'Fair', 'owned', 'First magnetic 2x2.', '2025-08-20', (select id from vendors v where v.slug = 'cubezz'), 7.49, 'XOF', 2840),
	('22222222-2222-4222-8222-222222222222', (select id from cube_models where slug = 'x-man-tornado-v4-pioneer'), false, 1, 'New', 'wanted', '', null, null, null, null, null);
